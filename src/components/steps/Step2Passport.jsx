    ...
    <div style={{ padding:'24px 28px' }}>
      ...
      <div className="form-grid-2">
        ...
        <div>
          ...
          <p className="field-hint">Phải còn hiệu lực ít nhất 6 tháng kể từ ngày nhập cảnh</p>
        </div>
        <div>
          <label className="field-label">Nơi cấp <span className="req">*</span></label>
          <input className="field-input" type="text" placeholder="Cục Xuất nhập cảnh TP.HCM"
        ...
        <div>
          ...
          <select className="field-input" value={data.issueCountry} onChange={e => onChange('issueCountry', e.target.value)}>
            {['Việt Nam','Mỹ','Nhật Bản','Hàn Quốc','Trung Quốc','Pháp','Đức'].map(c => <option key={c}>{c}</option>)}
          </select>
        </div>
      </div>
      ...
      >
        ...
        {data.passportImgURL ? (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:8 }}>
            ...
            <p style={{ fontSize:11, color:'#9CA3AF' }}>Nhấn để thay đổi</p>
          </div>
        ) : (
          <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:6 }}>
            ...
            <p style={{ fontSize:14, fontWeight:600, color:'#374151' }}>Tải ảnh trang thông tin hộ chiếu</p>
            <p style={{ fontSize:12, color:'#9CA3AF' }}>Ảnh rõ nét, đủ 4 góc · JPG, PNG · Tối đa 5MB</p>
          </div>
        )}
      </div>
